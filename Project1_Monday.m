            ...
            switch location
                ...
                case 'PML_Bottom'
                    ...
                    H_z(1,i,j) = ((2*delt)/(delta*(2*mu+sigma_mx*delt)))*...
                      ...
                      ((delt*sigma_mx)/(2*mu+sigma_mx*delt))*H_z(2,i,j);
                    % Finite Difference Equation (3) from our notes
                    H_y(1,i,j+1) = -1*((2*delt)/(delta*(2*mu+sigma_my*delt)))*...
                      (E_xz(2,i,j+1)-E_xz(2,i,j)+E_xy(2,i,j+1)-E_xy(2,i,j)) + ...
                      (2*mu/(2*mu+sigma_my*delt))*H_y(2,i,j+1) - ...
                      ((delt*sigma_my)/(2*mu+sigma_my*delt))*H_y(2,i,j+1);    
                    % Finite Difference Equation (2) from our notes
                    E_xy(1,i,j+1) = -1*((2*delt)/(delta*(2*pml_e+sigma_ey*delt)))*...
                    (H_y(1,i,j+1)-H_y(1,i,j)) + ...
                    (2*pml_e/(2*pml_e+sigma_ey*delt))*E_xy(2,i,j+1) - ...
                    ((delt*sigma_ey)/(2*pml_e+sigma_ey*delt))*E_xy(2,i,j+1);                                   
                    % Finite Difference Equation (1) from our notes
                    E_xz(1,i,j-1) = ((2*delt)/(delta*(2*pml_e+sigma_ex*delt)))*...
                    (H_z(1,i,j-1)-H_z(1,i-1,j-1)) + ...
                    (2*pml_e/(2*pml_e+sigma_ex*delt))*E_xz(2,i,j-1) - ...
                    ((delt*sigma_ex)/(2*pml_e+sigma_ex*delt))*E_xz(2,i,j-1);                      
                case 'PML_Top'
                    pml_e = e_top;                              
                    sigma_mx = sigma_x;
                    sigma_ex = pml_e*sigma_mx/mu;
